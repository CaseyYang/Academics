...
namespace RTree
{
    public class RTree<T>
    {
        ...
        /// 得到R树索引中的所有MBR
        /// </summary>
        /// <returns>返回所有MBR矩形框</returns>
        public List<Rectangle> GetMBRs()
        {
            List<Rectangle> resultList = new List<Rectangle>();
            foreach (KeyValuePair<int, Node<T>> pair in nodeMap)
            {
                if (!deletedNodeIds.Contains(pair.Key))
                {
                    resultList.Add(pair.Value.getMBR());
                }
            }
            return resultList;
        }
        ...
        private bool delete(Rectangle r, int id)
        {
            ...
            // shrink the tree if possible (i.e. if root Node<T> has exactly one entry,and that 
        ...
        /// <summary>
        /// 在给定最远距离的情况下，找出离查询点最近的元素
        /// </summary>
        /// <param name="p">查询点</param>
        /// <param name="furthestDistance">最远距离</param>
        /// <returns>最近邻集合</returns>
        ...
        /// 得到R树中位于给定矩形范围内的所有元素
        /// </summary>
        /// <param name="r">给定矩形范围</param>
        /// <returns>结果元素集合</returns>
